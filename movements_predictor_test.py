...
class Predictor:
    ...
    def __sliding_window(self, w_ml, w_history):
        ...
        for i in range(0, len(w_history)-w_ml):
    ...
    def __sliding_window_average(self, w_avg, train_set):
        ...
        for i in range(0, len(train_set)-w_avg):
    ...
    def __weighted_accuracy(self, actual, predicted):
        '''
        ...
        for i in range(len(actual)):
            difference += abs(actual[i] - predicted[i])
    ...
    def __multi_step_prediction(self, w_history, w_ml, regressor):
        ...
        return self.__weighted_accuracy(self.__test_set, predictions), predictions
    ...
    def __simulation(self, w_ml, w_history, mdl):
        ...
        return self.__multi_step_prediction(w_history, w_ml, regressor)
    ...
    def find_best_model(self, coordinate_history, name, lock, verbosity=1):
        ...
        for w_avg in range(0, 4):
            ...
            for w_ml in range(1, len(w_history)-3):
                for pol_deg in range(1, 4):
                    ...
                    if(self.__max_accuracy < 100):
                        accuracy, predictions = self.__simulation(w_ml, w_history, pipeline)
    ...
    #   Normalize percentage of lists within the range [0 100]
    def normalize_percentage(self, lock, name):
        l = []

        for i in range(len(self.__test_set)):
            result = abs(100 - abs(np.array(self.__test_set[i])-np.array(self.final_predictions[i])))
            l.append(result)

            if result < 75 or result > 100:    
                return l
